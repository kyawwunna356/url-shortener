...
import { InfoHeading } from './components/styled/InfoHeading.styled';
import InputForm from './components/InputForm';
import axios from 'axios';
import Card from "./components/Card";
import { InfoBody } from "./components/styled/InfoBody.styled";
import InfoCard from "./components/InfoCard";
import { Dash } from "./components/Dash.styled";
import { Booster } from "./components/styled/Booster.styled";
import { Footer } from "./components/styled/Footer.style";
import { UsefulLinks } from "./components/styled/UsefulLinks";
import { Icons } from "./components/styled/Icons.styled";
...
function App() {
  ...
  async function request(){
    try {
      const res = await axios.get(`https://api.shrtco.de/v2/shorten?url=${input}`);
      const link = await res.data.result.full_short_link;
      setUrl((prevState) => {
        return [
          ...prevState,
          {
  ...
  return (
    <div className="App">
      ...
      <GlobalStyle/>
          ...
          <Hero>
            <Flex>
              <div>
                ...
                <p>Build your brand's recongnition and get detailed <br/>insights on how your links are performing.</p>
                <Button color={theme.color.button}>Get started</Button>
              </div>
              <div>
        ...
        <InputForm error={error} input={input} handleInput={handleInput} shorten={shorten} valid={valid} ></InputForm>
        

        
        <Info>
          {
            url.length > 0 ? url.map(el => <Card key={el.id} url={el.url} short={el.short} copied={el.copied} handleCopied={() => {funcCopied(el.id)}}/>) : ''
          }
            <InfoHeading>
                <h1>Advanced Statistics</h1>
                <p>Track how your links are performing across the web with an advanced statistics dashboard</p>
            </InfoHeading>
            <InfoBody>
              <InfoCard>
                  <div><img src="./images/icon-brand-recognition.svg" alt="" /></div>
